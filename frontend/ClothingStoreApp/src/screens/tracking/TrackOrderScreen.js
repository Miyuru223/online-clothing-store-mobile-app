        ...
        <View style={styles.resultBox}>
          ...
          {!isCancelled ? (
              ...
              {STEPS.map((step, index) => {
                ...
                return (
                  <View key={step} style={styles.stepRow}>
                    {/* Line above (skip first) */}
                    {index > 0 && (
                      <View style={[styles.lineAbove, { backgroundColor: index <= currentStep ? STATUS_COLOR[STEPS[index - 1]] : '#eee' }]} />
                    )}
                    <View style={styles.stepLeft}>
                      <View style={[styles.stepCircle, { backgroundColor: color, borderColor: color }]}>
                        <Text style={styles.stepIcon}>{done ? STATUS_ICON[step] : '○'}</Text>
                      </View>
                    </View>
                    <View style={[styles.stepContent, active && styles.stepContentActive]}>
                      <Text style={[styles.stepName, { color: done ? '#222' : '#aaa', fontWeight: active ? '700' : '500' }]}>
                        {step}
                      </Text>
                      {active && (
            ...
            <View style={styles.cancelledBox}>
              ...
              <Text style={styles.cancelledTxt}>This order has been cancelled</Text>
...
const styles = StyleSheet.create({
  ...
  orderIdTxt: { fontSize: 16, fontWeight: '700', color: '#222' },
  ...
  lineAbove: { position: 'absolute', left: 17, top: -12, width: 2, height: 14 },
  stepLeft: { width: 36, alignItems: 'center', marginRight: 12 },
  stepCircle: { width: 36, height: 36, borderRadius: 18, borderWidth: 2, alignItems: 'center', justifyContent: 'center' },
  stepIcon: { fontSize: 16 },
  stepContent: { flex: 1, paddingBottom: 16, borderBottomWidth: 1, borderBottomColor: '#f0f0f0' },
  stepContentActive: { borderBottomColor: 'transparent' },
  stepName: { fontSize: 15 },
  stepDesc: { fontSize: 12, marginTop: 3, lineHeight: 18 },
  ...
  cancelledIcon: { fontSize: 48, marginBottom: 10 },
  cancelledTxt: { fontSize: 17, fontWeight: '700', color: '#EF4444' },
